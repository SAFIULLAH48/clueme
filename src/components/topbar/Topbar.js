import { Chat, Notifications, Person, Search, Widgets } from "@mui/icons-material";
//these icons are taken from material 
import "./topbar.css";

const Topbar = () => {
  return (
    <div className="topbarContainer">
      <div className="topbarLeft"   /* this container is for top left on appbar which is Logo or Name */>
   
        
        <span className="logo">ClueMe</span>
      </div>
      <div className="topbarCenter">
        <div className="searchbar">
          / this container is for center which is consists of searchbar */
          <Search className="searchicon"/  /* icon is taken from material ui */ >
     
          <input placeholder="search here" className="searchInput" />
        </div>
      </div>
      <div className="topbarRight"   /* this container is for top right */>
       
       
        <div className="topbarIcons" >
          <div className="topbaritemIcon">
            <Person / /* icons from material ui */>
            <span className="topbarIconbadge"  /* here we used icon badge which is red circle notification on icons */>1</span>
          </div>
          <div className="topbaritemIcon">
            <Chat />
            <span className="topbarIconbadge">7</span>
          </div>
          <div className="topbaritemIcon">
            <Notifications />
            <span className="topbarIconbadge">4</span>
          </div>
          <div className="topbaritemIcon">
            <Widgets />
           
          </div>
          <img /* images are directly taken from google   this image is used for user image */
            src="data:image/png;base64,"
            alt="error"
            className="topbarImg"
          />
        </div>
      </div>
    </div>
  );
};

export default Topbar;
